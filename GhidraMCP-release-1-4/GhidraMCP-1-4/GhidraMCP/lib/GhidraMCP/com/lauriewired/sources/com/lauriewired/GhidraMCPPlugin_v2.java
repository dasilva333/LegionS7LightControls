...
public class GhidraMCPPlugin extends Plugin {
    ...
    private void startServer() throws IOException {
        ...
        // v2 REST-style routes used by bridge_mcp_ghidra_v2.py
        server.createContext("/function/", this::handleFunctionV2);
        server.createContext("/patch/", this::handlePatchV2);
        server.createContext("/types", this::handleTypesV2);
        server.createContext("/type/", this::handleTypeV2);
        server.createContext("/data/", this::handleDataV2);
        server.createContext("/search/", this::handleSearchV2);
        server.createContext("/tags", this::handleTagsV2);
        server.createContext("/bookmarks", this::handleBookmarksV2);
 ...
 */
    ...
    // ===================== v2 handlers =====================
    private void handleFunctionV2(HttpExchange exchange) throws IOException {
        Program program = getCurrentProgram();
        if (program == null) { sendResponse(exchange, "No program loaded"); return; }
        String path = exchange.getRequestURI().getPath(); // /function/{id}/{action}
        String[] parts = path.split("/");
        if (parts.length < 4) { sendResponse(exchange, "Invalid function request path"); return; }
        String id = parts[2];
        String action = parts[3];

        Function func = resolveFunction(program, id);
        if (func == null) { sendResponse(exchange, "Function not found: " + id); return; }

        switch (action) {
            case "callers": sendResponse(exchange, listCallers(program, func)); break;
            case "callees": sendResponse(exchange, listCallees(program, func)); break;
            case "cfg":     sendResponse(exchange, buildCfg(program, func)); break;
            case "pcode":   sendResponse(exchange, listPcode(program, func, false)); break;
            case "high_pcode": sendResponse(exchange, listPcode(program, func, true)); break;
            default: sendResponse(exchange, "Unknown action");
        }
    }

    private Function resolveFunction(Program program, String identifier) {
        try {
            Address a = program.getAddressFactory().getAddress(identifier);
            Function f = program.getFunctionManager().getFunctionAt(a);
            if (f != null) return f;
            return program.getFunctionManager().getFunctionContaining(a);
    ...
    private String listCallers(Program program, Function func) {
        ...
        while (it.hasNext()) {
            ...
            Function from = program.getFunctionManager().getFunctionContaining(r.getFromAddress());
            if (from != null) names.add(from.getName());
        }
        ...
        while (it.hasNext()) {
            ...
            if (refs != null) {
                for (Reference r : refs) {
                    if (!r.getReferenceType().isCall()) continue;
                    Function to = program.getFunctionManager().getFunctionAt(r.getToAddress());
                    if (to != null) names.add(to.getName());
                }
            }
        }
        List<String> out = new ArrayList<>(names);
    ...
    private String buildCfg(Program program, Function func) {
        try {
            ...
            ghidra.program.model.block.CodeBlockIterator bit = model.getCodeBlocksContaining(func.getBody(), new ConsoleTaskMonitor());
            List<String> nodes = new ArrayList<>();
            List<String> edges = new ArrayList<>();
            List<ghidra.program.model.block.CodeBlock> blocks = new ArrayList<>();
            while (bit.hasNext()) { blocks.add(bit.next()); }
            for (ghidra.program.model.block.CodeBlock b : blocks) {
                String start = b.getMinAddress().toString();
                String end = b.getMaxAddress().toString();
                nodes.add("{\"id\":\""+start+"\",\"start\":\""+start+"\",\"end\":\""+end+"\"}");
            }
            for (ghidra.program.model.block.CodeBlock b : blocks) {
                String from = b.getMinAddress().toString();
                ghidra.program.model.block.CodeBlockReferenceIterator dit = b.getDestinations(new ConsoleTaskMonitor());
                while (dit.hasNext()) {
                    ghidra.program.model.block.CodeBlockReference ref = dit.next();
                    String to = ref.getDestinationBlock().getMinAddress().toString();
                    edges.add("{\"from\":\""+from+"\",\"to\":\""+to+"\"}");
                }
            }
            return "{\"nodes\":[" + String.join(",", nodes) + "],\"edges\":[" + String.join(",", edges) + "]}";
        } catch (Exception e) {
            return "{\"error\":\"cfg failed: " + e.getMessage().replace("\"","'") + "\"}";
        }
    }
    ...
    private String listPcode(Program program, Function func, boolean high) {
        ...
        try {
            ...
            DecompileResults res = di.decompileFunction(func, 30, new ConsoleTaskMonitor());
            if (!res.decompileCompleted()) return "Decompile failed: " + res.getErrorMessage();
            HighFunction hf = res.getHighFunction();
            List<String> out = new ArrayList<>();
            Iterator<?> it = high ? hf.getPcodeOps() : hf.getPcodeOps(func.getEntryPoint());
            while (it.hasNext()) out.add(it.next().toString());
            return String.join("\n", out);
        } finally { di.dispose(); }
    }

    private void handlePatchV2(HttpExchange exchange) throws IOException {
        String path = exchange.getRequestURI().getPath(); // /patch/instruction/{addr}
        String[] parts = path.split("/");
        if (parts.length >= 4 && "instruction".equals(parts[2])) {
            String addressStr = parts[3];
            String body = new String(exchange.getRequestBody().readAllBytes(), StandardCharsets.UTF_8);
            String hex = extractJsonStringField(body, "bytes");
            if (hex == null) { sendResponse(exchange, "Missing 'bytes'"); return; }
            Program program = getCurrentProgram();
            if (program == null) { sendResponse(exchange, "No program loaded"); return; }
            byte[] bytes = new byte[hex.length()/2];
            try { for (int i=0;i<bytes.length;i++){ bytes[i]=(byte)Integer.parseInt(hex.substring(i*2,i*2+2),16);} }
            catch (Exception e) { sendResponse(exchange, "Invalid hex"); return; }
            AtomicBoolean ok = new AtomicBoolean(false);
            try {
                SwingUtilities.invokeAndWait(() -> {
                    int tx = program.startTransaction("Patch Instruction");
                    try {
                        Address addr = program.getAddressFactory().getAddress(addressStr);
                        program.getMemory().setBytes(addr, bytes);
                        ok.set(true);
                    } catch (Exception e) { Msg.error(this, "patch failed", e); }
                    finally { program.endTransaction(tx, ok.get()); }
                });
            } catch (Exception e) { sendResponse(exchange, "Patch failed: " + e.getMessage()); return; }
            sendResponse(exchange, ok.get()?"Patched":"Failed");
            return;
        }
        sendResponse(exchange, "Not Implemented");
    }

    private void handleTypesV2(HttpExchange exchange) throws IOException { sendResponse(exchange, "\n"); }
    private void handleTypeV2(HttpExchange exchange) throws IOException { sendResponse(exchange, "Not Implemented"); }
    private void handleDataV2(HttpExchange exchange) throws IOException { sendResponse(exchange, "Not Implemented"); }
    private void handleSearchV2(HttpExchange exchange) throws IOException { sendResponse(exchange, "Not Implemented"); }
    private void handleTagsV2(HttpExchange exchange) throws IOException { sendResponse(exchange, "\n"); }
    private void handleBookmarksV2(HttpExchange exchange) throws IOException { sendResponse(exchange, "Not Implemented"); }
